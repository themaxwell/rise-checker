...
// Tail rise.log
	$last = tailCustom($riselog, $linestoread);
...
// If counter + current count is greater than $max_count, take action...
    if (($counter + $count) >= $max_count) {
        ...
        // If rise-snapshot directory exists..
        if(file_exists($snapshotDir)){
          ...
          passthru("cd $snapshotDir && echo y | ./rise-snapshot.sh restore");
...
// If counter + current count is not greater than $max_count, add current count to our database...
    	...
    	if(($counter + $count) < $max_count && $createsnapshot === true){
    		...
    		// Check if path to rise-snapshot exists..
        if(file_exists($snapshotDir)){
          
          $snapshots = glob($snapshotDir.'snapshot/rise_db'.date("d-m-Y").'*.snapshot.tar');
          if (!empty($snapshots)) {
            ...
            echo "\t\t\tGoing to remove snapshots older than $max_snapshots days...\n";
              $files = glob($snapshotDir.'snapshot/rise_db*.snapshot.tar');
            ...
            ob_start();
            $create = passthru("cd $snapshotDir && ./rise-snapshot.sh create");
          ...
          // Path to rise-snapshot does not exist..
          echo "\t\t\tYou have rise-snapshot enabled, but the path to rise-snapshot does not seem to exist.\n
                \t\t\tDid you install rise-snapshot?\n";
        }
